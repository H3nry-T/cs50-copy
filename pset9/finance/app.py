...
def buy():
    ...
    if not request.form.get("shares"):
        ...
        new_value_row = db.execute("SELECT value_of_stock FROM history WHERE history_user_id = ? ORDER BY value_of_stock ASC LIMIT 1", session["user_id"])
        old_value_row = db.execute("SELECT value_of_stock FROM history WHERE history_user_id = ? ORDER BY value_of_stock ASC LIMIT 2", session["user_id"])
        total_value_of_stock = new_value_row[0]["value_of_stock"] + old_value_row[0]["value_of_stock"]
